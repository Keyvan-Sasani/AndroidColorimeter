...
import com.armin.caloriemeter.LazyList.ImageLoader;
import com.armin.caloriemeter.LazyList.LazyAdapter;
import com.armin.caloriemeter.R;
import com.armin.caloriemeter.LoadButForget.ImageAdapter;
...
//        adapter = new ResultsAdapter(this, results, results2);
//        adapter = new CustomListAdapter(this, results, results2);
        adapter = new ImageAdapter(this, results, results2);
//        adapter = new LazyAdapter(this, results, results2);
...
//        if (query == null){
//       }
        new FoodQuery().executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR,s);
//        query = new FoodQuery();
//        query.execute(s);
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        String urlName = results2.get(position);
        ...
        do {
            ...
            url[i] = cursor.getString(cursor.getColumnIndexOrThrow(FoodEntry.COLUMN_NAME_URL));
            i++;
        ...
        if (searchMode == Constants.INGREDIENTS) // creating new food
        {
            ...
            dialog.setArguments(unit, energy, amount, url);
...
//			startActivity(intent);
    ...
    class FoodQuery extends AsyncTask<Object, Object, Boolean> {
        ...
        protected Boolean doInBackground(Object... params) {
            ...
            SQLiteDatabase db = mDbHelper.getReadableDatabase();
            String[] projection1 = {FoodEntry.COLUMN_NAME_FOOD_NAME, FoodEntry.COLUMN_NAME_URL};
            ...
            do {
                ...
                String url_name = cursor.getString(
                        cursor.getColumnIndexOrThrow(
                                FoodEntry.COLUMN_NAME_URL));
                if (!results.contains(Constants.MARKED + name))
                {results.add(name);
                    results2.add(url_name);
                }
            } while (cursor.moveToNext());
    ...
    @Override
    public void onDialogPositiveClick(int amount, int energy, String unit, int listItemPosition, String url) {
        results.set(listItemPosition, Constants.MARKED + results.get(listItemPosition));
        adapter.notifyDataSetChanged();
        markedItems.add(new MarkedItem(results.get(listItemPosition), amount, unit, energy, url));
        energyTotal += energy;
    }

    public class MarkedItem {
        public MarkedItem(String name, int amount, String unit, int energy, String url) {
            this.name = name;
            this.amount = amount;
            this.unit = unit;
            this.energy = energy;
